...
public class LogoutController {
	...
	@RateLimit( key = "LOGOUT_",
			    maxRequests = 3,
			    timeFrameInMinutes = 15,
			    identifiers = {IdentifierType.USER, IdentifierType.IP})
	@PostMapping("/auth/logout")
	public ResponseEntity<LogoutResponse> logout(@RequestBody LogoutRequest logoutRequest, HttpServletRequest httpServletRequest) {
		return ResponseEntity.ok(logoutService.logout(logoutRequest,httpServletRequest));
	}
