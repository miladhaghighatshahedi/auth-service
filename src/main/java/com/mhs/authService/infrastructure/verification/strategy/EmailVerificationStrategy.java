...
class EmailVerificationStrategy implements VerificationStrategy {
	...
	public boolean supports(UsernameType type) {
		return type == UsernameType.EMAIL;
	}

	@Override
	public VerificationPayload generate(String email) {
		String token = jwtVerificationTokenGenerator.generate(email);
		return new VerificationPayload(token,email,UsernameType.EMAIL);
	}
